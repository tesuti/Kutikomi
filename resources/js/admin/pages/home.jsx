                    ...
                    <div className='flex pt-3'>
                        ...
                        <div className=''>
                            {posts.slice(0,visible).map((posts, i)=>(
                                <div  key={i}>
                                <div className='flex gap-3 mt-3 hover:bg-slate-200 hover:shadow-sm rounded-xl'>
                                    <p>{++i}</p>
                                            <Link to={{ pathname :"/admin/view/"+posts.id }}>
                                                <div className='pt-3 grid lg:grid-cols-4 md:grid-cols-2 grid-cols-1 gap-6'>
                                                <div className=''>
                                                    <img src={ "http://127.0.0.1:5173/storage/app/public/images/" +posts.photo}  alt={posts.photo}  className='rounded-xl aspect-video object-cover w-40'/>
                                                </div>
                                                    <h3 className='text-base home__posts-body break-all w-64'>{ posts.title}</h3>
                                                    <p className='truncate text-sm font-normal text-gray-600 w-40'>{ posts.body}</p>
                                                </div>
                                            </Link>
                                        <div  className='flex justify-center items-right gap-3 pt-4  '>
                                            {/* <Link to={{ pathname :"edit/"+posts.id }}>
                                            <button className='' type='button'> 編
                                            </button>
                                            </Link> */}
                                            <div onClick={() =>{setPopup(posts.id)}}>
                                            <button type="button" className="mr-2.5 text-xl hover:text-slate-400" onClick={() => setOpenDelete(!openDelete)}
                                            >
                                            <HiTrash />
                                            </button>
                                            </div>
